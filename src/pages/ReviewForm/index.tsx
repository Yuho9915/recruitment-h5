...
export const ReviewForm = () => {
  ...
  const [formData, setFormData] = useState({
    audioTextFile: null as File | null,
    jobName: '',
    jobDesc: '',
    experience: '1-3年工作经验',
    interviewerRole: 'HR面试官',
    customRole: '',
    resume: null as File | null
  });

  const [audioTextFileName, setAudioTextFileName] = useState('');
  ...
  const handleAudioTextUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setFormData((prev) => ({ ...prev, audioTextFile: file }));
      setAudioTextFileName(file.name);
    }
  };
  ...
  const handleStartReview = () => {
    console.log('Start review with data:', formData);
        ...
        <div className="px-4 py-4 space-y-4">
          ...
          <div className="bg-white rounded-lg p-4">
            ...
            <label className="block">
              <input
                type="file"
                onChange={handleAudioTextUpload}
                className="hidden"
                accept=".pdf,.doc,.docx"
              />
            ...
            <p className="text-xs text-text-tertiary mt-2">
              支持格式：PDF、DOCX，文件大小不超过10MB
            </p>
          </div>

          {/* Target Job Info Section */}
          <div className="bg-white rounded-lg p-4">
            <h3 className="text-base font-semibold text-text-primary mb-4">
              目标岗位信息
            </h3>
